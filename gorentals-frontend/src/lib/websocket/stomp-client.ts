...
export function useStomp() {
  ...
  useEffect(() => {
    ...
    client.onStompError = (frame) => {
      if (process.env.NODE_ENV === "development") console.error('[STOMP] Broker reported error: ' + frame.headers['message']);
      if (process.env.NODE_ENV === "development") console.error('[STOMP] Additional details: ' + frame.body);
    };
  ...
  const subscribe = useCallback((destination: string, callback: (message: IMessage) => void): () => void => {
    if (!clientRef.current || !clientRef.current.connected) {
      ...
      if (process.env.NODE_ENV === "development") console.warn('[STOMP] Client not connected yet, subscription may fail');
    }
