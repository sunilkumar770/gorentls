...
// ── Types — field names match BookingResponse.java getters exactly ───────────
...
// ── Status config — keys match backend BookingStatus enum exactly ────────────
...
const TERMINAL: OwnerBooking['status'][] = ['COMPLETED', 'CANCELLED', 'REJECTED'];
...
export default function OwnerBookingsPage() {
  ...
  const fetchBookings = useCallback(async () => {
    try {
      ...
      // Backend returns Spring Page<BookingResponse> → { content: [...] }
      ...
      setError(
        ...
        'Could not load bookings. Check that the backend is running.'
      );
    } finally {
      setLoading(false);
    }
        ...
        {!loading && !error && bookings.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-gray-500 mt-2 text-sm max-w-sm mx-auto">
              When renters request your listings, they will appear here for you to accept or reject.
            </p>
          </div>
        )}
        ...
        {!loading && !error && bookings.length > 0 && (
          <div className="space-y-4">
            {bookings.map(b => {
              ...
              const canComplete = b.status === 'CONFIRMED' || b.status === 'ACTIVE';
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-gray-500 mt-1">
                        ...
                        {b.renter?.phone && (
                          <span className="ml-2 text-gray-400">· {b.renter.phone}</span>
                        )}
                      </p>
                        ...
                        <>
                          <button
                            ...
                            className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white
                                       text-sm font-medium rounded-lg disabled:opacity-50
                                       transition-colors"
                          >
                          ...
                          <button
                            ...
                            className="px-4 py-2 bg-white text-red-700 border border-red-200
