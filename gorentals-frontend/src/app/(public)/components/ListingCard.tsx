...
export function ListingCard({ item }: { item: Listing }) {
  ...
  return (
    <Link
      ...
      className="group block rounded-3xl border border-border-subtle bg-surface-base overflow-hidden hover:border-brand-300 hover:shadow-xl transition-all duration-300"
    >
      {/* Image Container */}
      <div className="relative aspect-[4/3] bg-surface-subtle overflow-hidden">
        ...
        {/* Condition Badge */}
        {item.condition && (
          <span className={`absolute top-4 left-4 text-[10px] uppercase tracking-widest font-bold px-2.5 py-1 rounded-lg backdrop-blur-md ${CONDITION_STYLES[item.condition] ?? 'bg-surface-base/90 text-text-primary'}`}>
            {item.condition}
          </span>
        )}
      </div>
      ...
      <div className="p-5 space-y-4">
        <div>
          <h3 className="font-bold text-text-primary text-base leading-tight line-clamp-1 group-hover:text-brand-600 transition-colors">
            {item.title}
          </h3>
        ...
        <div className="flex items-center justify-between pt-1">
          <div className="flex items-baseline gap-1">
            ...
            <span className="text-xs font-bold text-text-secondary uppercase tracking-tighter">/ day</span>
          </div>
        </div>

        {/* Owner Info */}
        <div className="flex items-center gap-3 pt-4 border-t border-border-subtle">
          <div className="w-8 h-8 rounded-full bg-brand-100 dark:bg-brand-900 flex items-center justify-center text-brand-700 dark:text-brand-300 text-xs font-black shrink-0">
            {ownerInitial}
          </div>
