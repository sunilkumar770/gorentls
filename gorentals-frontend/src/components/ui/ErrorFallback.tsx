...
export function ErrorFallback({ error, resetErrorBoundary }: { error: any; resetErrorBoundary: () => void }) {
  return (
    <div className="bg-[var(--bg-card)] rounded-[var(--r-xl)] shadow-sm border border-[#CCFBF1] p-12 text-center shadow-card">
      <div className="w-16 h-16 bg-[#CCFBF1] rounded-full flex items-center justify-center mx-auto mb-5">
        <ShieldAlert className="w-8 h-8 text-[#115E59]" />
      </div>
      ...
      <button
        ...
        className="inline-flex h-12 items-center justify-center gap-2 px-8 bg-[#115E59] text-white text-sm font-bold rounded-[var(--r-md)] shadow-card hover:bg-[#991b1b] transition-all"
      >
